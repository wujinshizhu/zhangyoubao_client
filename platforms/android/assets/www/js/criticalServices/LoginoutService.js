...
//该服务用于登录和登出的相关逻辑处理
rootService.factory('LoginoutService',function($ionicPopup,$cordovaToast,$http,LogOutService,ImgPathService,
                                               SocketService,ChatCacheService,UserNameService,
                                               $rootScope,LocalStorage,NetworkStateService,$state,Bank,$ionicLoading){
    ...
    {
        if(typeof($data.storeInfo)!='undefined')
        {
            $rootScope.storeInfo=$data.storeInfo;
            console.log($rootScope.storeInfo);
        }
    };

    var setFocusInfo=function($user_data){
        if($user_data.focus_trade_ids!=null)
        {
            $user_data.focus_trade_ids=$user_data.focus_trade_ids.split(',');
        }
        if($user_data.focus_store_ids!=null)
        {
            $user_data.focus_store_ids=$user_data.focus_store_ids.split(',');
        }
    };
    ...
    var onLoginSuccess=function(data,password){
        ...
        saveLoginInfo(password);
        //登录聊天服务器
        SocketService.loginChatServer();
        //读取本地聊天缓存
        ChatCacheService.readChatCache($rootScope.userInfo.user_id);
    };
    //退出登录
    var logout=function(){
        //保存该用户的聊天记录
