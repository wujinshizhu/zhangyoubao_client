...
rootModule.controller('SystemMsgListCtrl',function($scope,$rootScope,ChatCacheService,
      ...
      $scope.onMsgClick = function(index){
          ...
          else if (msg.type == $rootScope.MSG_TYPE.APPLY_ACCEPTED){
            //交易确认, chat_id用于从服务器端获取聊天数据, status用于交易申请页面的状态处理, msg_index表明系统消息的index,用于获取该系统消息
            $state.go("transaction-apply",{chat_id:msg.chat_id, status:3, msg_index:index})
          }
      };
