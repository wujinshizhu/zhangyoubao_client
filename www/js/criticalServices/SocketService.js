...
// 	var myIoSocket = io.connect('http://chat.socket.io');
    ...
    function loginChatServer()
    {
        ...
        angularSocket.on('apply_success',function(){
          ...
          $rootScope.async_state = $rootScope.ASYNC_STATUS.SUCCESS;
        });

        //交易申请失败
        angularSocket.on('apply_error',function(){
          console.log('apply_error');
          $rootScope.async_state = $rootScope.ASYNC_STATUS.FAILED;
        });

        angularSocket.on('apply_accept_success',function(){
          //将申请状态设置为成功
          console.log('apply_accept_success');
          $rootScope.async_state = $rootScope.ASYNC_STATUS.SUCCESS;
        });

        //交易申请失败
        angularSocket.on('apply_accept_error',function(){
          console.log('apply_accept_error');
          $rootScope.async_state = $rootScope.ASYNC_STATUS.FAILED;
        });
        ...
        angularSocket.on('new_message',function(msg){
            ...
            if(!$rootScope.isActive)
            {
                ...
                var text = null;
                switch (msg.type) {
                  case $rootScope.MSG_TYPE.APPLICATION:
                    title = '您收到新的交易申请';
                    text = '系统消息: ' + msg.content;
                    break;
                  case $rootScope.MSG_TYPE.APPLY_ACCEPTED:
                    title = '您的交易申请已被接受';
                    text = '系统消息: ' + msg.content;
                    break;
                  default:
...
//      };
    ...
    function sendTransactionAccepted(apply){
        ...
        console.log("发送的交易确认");
        console.log(msg);
        angularSocket.emit('transaction_accepted', msg);
    }
