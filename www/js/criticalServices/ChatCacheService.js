...
rootModule.factory('ChatCacheService',function(LocalStorage,$rootScope,$ionicHistory,ImgPathService) {
    ...
    function removeSystemMsg(index){
      if(!chatCache.chatDic.hasOwnProperty($rootScope.SYSTEM_MSG_ID)) {
        return false;
      }
      else{
        chatCache.chatDic[$rootScope.SYSTEM_MSG_ID].chatRecords.splice(index,1);
      }
      updateUnReadMsgNumber();
      saveChatDic($rootScope.userInfo.user_id);
      return true;
    ...
    function updateSystemMsgUnread(){
        ...
        {
            var unReadNum = 0;
            for(var i =0; i<chatCache.chatDic[$rootScope.SYSTEM_MSG_ID].chatRecords.length; i++)
            {
                if(!chatCache.chatDic[$rootScope.SYSTEM_MSG_ID].chatRecords[i].has_read) {
                  unReadNum += 1;
                }
            }
            chatCache.chatDic[$rootScope.SYSTEM_MSG_ID].unReadNum = unReadNum;
            updateUnReadMsgNumber();
        }
    }
    ...
    function recevieMessage(obj, save)
    {
        ...
        {
            if(obj.type == $rootScope.MSG_TYPE.APPLICATION || obj.type == $rootScope.MSG_TYPE.APPLY_ACCEPTED) {
                // 接收到交易申请,插入一条系统消息
                //添加系统消息
                addSystemMsg(obj);
            }
    ...
    return {
        ...
        updateSystemMsgUnread: updateSystemMsgUnread,
        removeSystemMsg: removeSystemMsg,
        clearSystemMsg: clearSystemMsg
    }
